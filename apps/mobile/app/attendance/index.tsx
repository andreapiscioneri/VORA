...
import type { AttendanceEntry } from '@vora/shared/types/attendance'
...
export default function AttendanceScreen() {
  ...
  const { entries, loading, loadingMore, error, hasMore, reload, loadMore } = useAttendance()

  return (
    <DetailScreen
      title={t('modules.attendance.title')}
      subtitle={t('modules.attendance.count', { count: entries.length })}
      headerRight={
        <Pressable
          onPress={() => {
            haptics.tap()
          ...
          accessibilityLabel={t('modules.attendance.form.newTitle')}
          hitSlop={12}
        >
    ...
    >
      {error ? (
        <StateMessage text={t('modules.attendance.error', { error })} />
        ...
        <FlatList
          ...
          ListEmptyComponent={!loading ? <StateMessage text={t('modules.attendance.empty')} /> : null}
          onEndReached={hasMore ? loadMore : undefined}
          onEndReachedThreshold={0.5}
          ListFooterComponent={loadingMore ? <ActivityIndicator style={styles.footer} color={colors.primary} /> : null}
          renderItem={({ item }: { item: AttendanceEntry }) => (
            <Pressable
              onPress={() => {
                haptics.tap()
              ...
              style={styles.row}
              accessibilityRole="button"
              accessibilityLabel={`${item.employeeName}, ${item.date}, ${item.checkIn} - ${item.checkOut}`}
            >
