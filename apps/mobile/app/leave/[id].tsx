...
export default function EditLeaveRequestScreen() {
  ...
  if (!request) {
    return (
      <DetailScreen title={t('modules.leave.title')}>
        <StateMessage text={t('modules.leave.empty')} />
      </DetailScreen>
    )
  }
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      await update(id, result.data as LeaveRequestInput)
      haptics.success()
      router.back()
    } catch {
      haptics.error()
  ...
  function confirmDelete() {
    haptics.warning()
    Alert.alert(t('modules.leave.form.delete'), t('modules.leave.deleteConfirm'), [
      { text: t('modules.leave.form.cancel'), style: 'cancel' },
      {
        text: t('modules.leave.form.delete'),
        style: 'destructive',
        onPress: async () => {
          try {
            await remove(id)
            ...
            haptics.error()
            setSaveError(t('modules.leave.errors.delete'))
          }
        },
      },
    ])
  ...
  return (
    <DetailScreen title={t('modules.leave.form.editTitle')}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          ...
          <Pressable style={styles.deleteButton} onPress={confirmDelete} accessibilityRole="button">
            ...
            <Text style={styles.deleteText}>{t('modules.leave.form.delete')}</Text>
          </Pressable>
        </ScrollView>
