...
export default function EditCampaignScreen() {
  ...
  if (!campaign) {
    return (
      <DetailScreen title={t('modules.marketing.title')}>
        <StateMessage text={t('modules.marketing.empty')} />
      </DetailScreen>
    )
  }
  ...
  async function submit() {
    ...
    const form = { name, subject, body, recipientCount: campaign!.recipientCount, status: campaign!.status, sentAt: campaign!.sentAt }
    ...
    setSaving(true)
    try {
      await updateCampaign(id, result.data as MarketingCampaignInput)
      haptics.success()
      router.back()
    } catch {
      haptics.error()
  ...
  function confirmDelete() {
    haptics.warning()
    Alert.alert(t('modules.marketing.form.delete'), t('modules.marketing.deleteConfirm'), [
      { text: t('modules.marketing.form.cancel'), style: 'cancel' },
      {
        text: t('modules.marketing.form.delete'),
        style: 'destructive',
        onPress: async () => {
          try {
            ...
            haptics.error()
            setSaveError(t('modules.marketing.errors.delete'))
          }
        },
      },
    ])
  ...
  return (
    <DetailScreen title={t('modules.marketing.form.editTitle')}>
