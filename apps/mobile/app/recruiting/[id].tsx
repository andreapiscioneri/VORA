...
export default function EditCandidateScreen() {
  ...
  const { candidates, update, remove } = useCandidates()
  ...
  if (!candidate) {
    return (
      <DetailScreen title={t('modules.recruiting.title')}>
        <StateMessage text={t('modules.recruiting.empty')} />
      </DetailScreen>
    )
  }
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      await update(id, result.data as CandidateInput)
      haptics.success()
      router.back()
    } catch {
      haptics.error()
  ...
  function confirmDelete() {
    haptics.warning()
    Alert.alert(t('modules.recruiting.form.delete'), t('modules.recruiting.deleteConfirm'), [
      { text: t('modules.recruiting.form.cancel'), style: 'cancel' },
      {
        text: t('modules.recruiting.form.delete'),
        style: 'destructive',
        onPress: async () => {
          try {
            await remove(id)
            ...
            haptics.error()
            setSaveError(t('modules.recruiting.errors.delete'))
          }
        },
      },
    ])
  ...
  return (
    <DetailScreen title={t('modules.recruiting.form.editTitle')}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          ...
          <Pressable style={styles.deleteButton} onPress={confirmDelete} accessibilityRole="button">
            ...
            <Text style={styles.deleteText}>{t('modules.recruiting.form.delete')}</Text>
          </Pressable>
        </ScrollView>
