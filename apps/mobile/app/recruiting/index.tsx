...
import type { Candidate } from '@vora/shared/types/candidate'
...
export default function RecruitingScreen() {
  ...
  const { candidates, loading, loadingMore, error, hasMore, reload, loadMore } = useCandidates()

  return (
    <DetailScreen
      title={t('modules.recruiting.title')}
      subtitle={t('modules.recruiting.count', { count: candidates.length })}
      headerRight={
        <Pressable
          onPress={() => {
            haptics.tap()
            router.push('/recruiting/new')
          }}
          style={styles.addButton}
          accessibilityRole="button"
          accessibilityLabel={t('modules.recruiting.form.newTitle')}
          hitSlop={12}
        >
    ...
    >
      {error ? (
        <StateMessage text={t('modules.recruiting.error', { error })} />
        ...
        <FlatList
          ...
          ListEmptyComponent={!loading ? <StateMessage text={t('modules.recruiting.empty')} /> : null}
          ...
          renderItem={({ item }: { item: Candidate }) => (
            <Pressable
              onPress={() => {
                haptics.tap()
                router.push(`/recruiting/${item.id}`)
              }}
              style={styles.row}
              accessibilityRole="button"
              accessibilityLabel={`${item.firstName} ${item.lastName}, ${item.role}, ${t(`modules.recruiting.stage.${item.stage}`)}`}
            >
              ...
              <View style={styles.badge}>
                <Text style={styles.badgeText}>{t(`modules.recruiting.stage.${item.stage}`)}</Text>
              </View>
            </Pressable>
          )}
        />
