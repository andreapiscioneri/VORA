...
export function useAppointments() {
  ...
  async function fetchAppointments() {
    ...
    error.value = null
    try {
      const page = await $apiFetch<PageResult<Appointment>>('/api/appointments')
  ...
  async function loadMore() {
    ...
    try {
      const page = await $apiFetch<PageResult<Appointment>>('/api/appointments', { query: { cursor: nextCursor.value } })
  ...
  async function createAppointment(input: AppointmentInput) {
    const created = await $apiFetch<Appointment>('/api/appointments', { method: 'POST', body: input })
  ...
  async function updateAppointment(id: string, input: AppointmentInput) {
    const updated = await $apiFetch<Appointment>(`/api/appointments/${id}`, { method: 'PUT', body: input })
