...
export function useAutomations() {
  ...
  async function fetchAutomations() {
    ...
    error.value = null
    try {
      const page = await $apiFetch<PageResult<Automation>>('/api/automations')
  ...
  async function loadMore() {
    ...
    try {
      const page = await $apiFetch<PageResult<Automation>>('/api/automations', { query: { cursor: nextCursor.value } })
  ...
  async function createAutomation(input: AutomationInput) {
    const created = await $apiFetch<Automation>('/api/automations', { method: 'POST', body: input })
    automations.value = [created, ...automations.value]
    return created
  }

  async function updateAutomation(id: string, input: AutomationInput) {
    const updated = await $apiFetch<Automation>(`/api/automations/${id}`, { method: 'PUT', body: input })
  ...
  async function runAutomation(id: string, contactId: string) {
    return await $apiFetch<AutomationRunResult>(`/api/automations/${id}/run`, { method: 'POST', body: { contactId } })
  }
