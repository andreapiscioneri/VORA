...
export function useCommunications() {
  ...
  async function fetchCommunications() {
    ...
    error.value = null
    try {
      const page = await $apiFetch<PageResult<Communication>>('/api/communications')
  ...
  async function loadMore() {
    ...
    try {
      const page = await $apiFetch<PageResult<Communication>>('/api/communications', { query: { cursor: nextCursor.value } })
  ...
  async function sendMessage(payload: {
    ...
    const result = await $apiFetch<{ communication: Communication }>('/api/communications/send', { method: 'POST', body: payload })
  ...
  async function setStatus(id: string, status: CommunicationStatus) {
    const updated = await $apiFetch<Communication>(`/api/communications/${id}`, { method: 'PUT', body: { status } })
    communications.value = communications.value.map((c) => (c.id === id ? updated : c))
    return updated
  }

  async function setLabels(id: string, labels: string[]) {
    const updated = await $apiFetch<Communication>(`/api/communications/${id}`, { method: 'PUT', body: { labels } })
    communications.value = communications.value.map((c) => (c.id === id ? updated : c))
    return updated
  }
