...
export function useContacts() {
  ...
  async function fetchContacts() {
    ...
    error.value = null
    try {
      const page = await $apiFetch<PageResult<Contact>>('/api/contacts')
  ...
  async function loadMore() {
    ...
    try {
      const page = await $apiFetch<PageResult<Contact>>('/api/contacts', { query: { cursor: nextCursor.value } })
  ...
  async function createContact(input: ContactInput) {
    const created = await $apiFetch<Contact>('/api/contacts', { method: 'POST', body: input })
    contacts.value = [created, ...contacts.value]
    return created
  }

  async function updateContact(id: string, input: ContactInput) {
    const updated = await $apiFetch<Contact>(`/api/contacts/${id}`, { method: 'PUT', body: input })
  ...
  async function addAttachment(contactId: string, title: string, url: string) {
    return await $apiFetch<Contact>(`/api/contacts/${contactId}/attachments`, { method: 'POST', body: { title, url } })
  }
