...
export function useOpportunities() {
  ...
  async function fetchOpportunities() {
    ...
    error.value = null
    try {
      const page = await $apiFetch<PageResult<Opportunity>>('/api/opportunities')
  ...
  async function loadMore() {
    ...
    try {
      const page = await $apiFetch<PageResult<Opportunity>>('/api/opportunities', { query: { cursor: nextCursor.value } })
  ...
  async function createOpportunity(input: OpportunityInput) {
    const created = await $apiFetch<Opportunity>('/api/opportunities', { method: 'POST', body: input })
    opportunities.value = [created, ...opportunities.value]
    return created
  }

  async function updateOpportunity(id: string, input: OpportunityInput) {
    const updated = await $apiFetch<Opportunity>(`/api/opportunities/${id}`, { method: 'PUT', body: input })
