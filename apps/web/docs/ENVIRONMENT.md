...
| `EMAIL_PROVIDER_API_KEY` | no | Enables a real `EmailProvider` instead of the mock | Unset → `MockEmailProvider` (records sends locally, nothing sent) |
...
No `AUTH_SECRET` separate from `NUXT_SESSION_PASSWORD` — `nuxt-auth-utils` owns session encryption end-to-end, there's no second auth layer to configure. No OAuth client id/secret variables — OAuth login (Google/Microsoft) is not implemented (see [SECURITY.md](./SECURITY.md#authentication)); only email/password exists today. No third-party AI/email/WhatsApp credentials are invented or pre-filled — every one of `AI_API_KEY` / `EMAIL_PROVIDER_API_KEY` / `WHATSAPP_API_KEY` is genuinely optional and the app runs completely functionally without any of them, using the mock/heuristic implementations described in [AI.md](./AI.md).
