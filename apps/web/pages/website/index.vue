...
const { sites, pending, error, hasMore, loadingMore, fetchSites, loadMore } = useMicrosites()
await fetchSites()
  ...
  <div class="space-y-6">
    <div class="flex flex-col tablet:flex-row tablet:items-center gap-4">
      <div>
        ...
        <p class="text-body text-ink-400 mt-1">{{ $t('website.createSite') }}</p>
      </div>
      <button
        class="tablet:ml-auto shrink-0 flex items-center gap-2 px-4 py-2 rounded-md text-body-sm font-medium bg-primary text-ink-950 hover:bg-primary-hover transition-colors"
        @click="openNewSite"
      >
    ...
    <div v-else-if="sites.length === 0" class="rounded-lg border border-dashed border-ink-200 dark:border-white/10 p-12 text-center">
      <h2 class="text-h4 font-medium">{{ $t('sites.empty.title') }}</h2>
      <p class="text-body-sm text-ink-400 mt-2">{{ $t('sites.empty.subtitle') }}</p>
      <button class="mt-4 px-4 py-2 rounded-md text-body-sm font-medium bg-primary text-ink-950 hover:bg-primary-hover transition-colors" @click="openNewSite">
        {{ $t('sites.empty.cta') }}
      </button>
    </div>
    ...
    <div v-if="!pending && hasMore" class="flex justify-center">
      <button
        ...
        @click="loadMore"
      >
        {{ loadingMore ? $t('sites.loadingMore') : $t('sites.loadMore') }}
      </button>
    </div>
